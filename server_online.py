...
class Server:
    ...
    def handle_join_game(self, client_info):
        ...
        if choice == "1":
            ...
            with self.lock:
                for gid, ldata in game_sessions.items():
                    required_players = 2 if ldata['type'] == '1' else 4
                    if len(ldata['players']) < required_players:
                        creator_id = gid
                        break
        ...
        with self.lock:
            session = game_sessions.get(creator_id)
            if session:
                client_info['event'] = threading.Event()
                session['pending_requests'].append(client_info)
            else:
                sock.sendall(b"Game no longer exists.")
                return

        print(f"[SERVER] Join request from {client_info['username']} was queued for lobby {creator_id}.")
        accepted = client_info['event'].wait(timeout=45.0)

        if not accepted:
            try:
                sock.sendall(b"Host did not respond in time.")
            except:
                pass
    # def handle_join_game(self, client_info):
    #     sock = client_info['socket']
